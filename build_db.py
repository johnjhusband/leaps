...
CREATE TABLE IF NOT EXISTS snapshots(snapshot_date TEXT, ticker TEXT, buy TEXT, golden_verdict TEXT, golden_pct REAL,
  golden_valid TEXT, fwd_pe REAL, eps_2y REAL, eps_now REAL, eps_growth_2y REAL, price_growth_2y REAL, pe_then REAL,
...
cols=['buy','golden_verdict','golden_pct','golden_valid','fwd_pe','eps_2y','eps_now','eps_growth_2y','price_growth_2y',
...
for r in rows:
    _text={'buy','golden_valid'}
    vals=[DATE, r['ticker']]+[ (r[k] if (k.endswith('verdict') or k in _text) else num(r[k])) for k in cols ]
