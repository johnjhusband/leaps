...
6. **`build_universe.py`** → `universe_full.csv`, `universe_fractional.csv`, **`buy_list.csv`**, `MARKET_DIRECTION.md`
   - de-dupe the union → drop no-data names → apply the **golden line** (`goldenline.py`) → apply the
     **forward-confirmation** and the **moat gate** (reads `moat_verdicts.csv`) → set the `buy` column →
     apply the **tradability gate** (US-listed + clean exchange-listed ADRs only; foreign-local listings
     dropped unless in `ADR_MAP`; **Switzerland excluded entirely** — see `BUY_DECISION.md §A′`) →
     write `buy_list.csv` (the `buy=Y` names, ADR tickers swapped in) + the market-direction deploy gauge +
     stock/bond allocation. **This is the step that produces the buy list.** Exact rule: `BUY_DECISION.md`.
